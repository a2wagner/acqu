 ...
 * TA2KinFitPhysics
 *
 * Physics class for testing purposes.
 */
...
void TA2KinFitPhysics::Reconstruct()
{
	...
	if (nParticles == N_FINAL_STATE) {  // proceed if there are 3 particles in total (2g and proton)
		...
		Double_t massTarget = fP4target[0].M();
		Int_t geantIdxParticle = 1;  // photon beam
		Double_t beam[4]  = {0., Tagged[0].GetP4().E()/1000., 0., 0.};  // mass, energy, x, y of the incident particle at z=0
		Double_t Sbeam[3] = {Tagged[0].GetSigmaE()/beam[1], Tagged[0].GetSigmaTheta(), Tagged[0].GetSigmaPhi()};  // dE/E(beam energy), sigma(beam x), sigma(beam y)
		Double_t target[2] = {0., 10.};  // z coordinate of the target center, length of the target
		if (!(err = KFit.Kfilbm(massTarget, geantIdxParticle, beam, Sbeam, target)))
			//PrintError("", "", EErrFatal);
			printf("Kfilbm error = %d\n", err);
		geantIdxParticle = 1;  // photon
		Double_t prop[6] = {0.};  // mass, kin. energy, theta, phi, cluster depth in the crystals, original cluster energy
		Int_t iPart = 0;
		while (iPart < nNeutral) {
			if (particles[neutralPart[iPart]].HasDetector(EDetPID))  // Cluster in Crystal Ball
				cluster = particles[neutralPart[iPart]].FindCluster("CB", "NaI");
			else if (particles[neutralPart[iPart]].HasDetector(EDetVeto))  // Cluster must be in TAPS
				cluster = particles[neutralPart[iPart]].FindCluster("TAPS", "BaF2");
			else {  // this should not happen
				PrintError("", "Cluster not found", EErrFatal);
				// fprintf(stderr, "Cluster for particle %d in event %d not found!", iPart+1, gAN->GetNEvent());
				// exit(EXIT_FAILURE);
			}
			prop[1] = particles[neutralPart[iPart]].GetP4().E();
			prop[2] = particles[neutralPart[iPart]].GetP4().Theta();
			prop[3] = particles[neutralPart[iPart]].GetP4().Phi();
			prop[4] = Cluster depth...
			prop[5] = cluster->GetEnergy();
			if (!(err = KFit.Kfilcst(++iPart, 1, prop)))
				printf("Kfilcst error = %d\n", err);
		}
		
		KFit.Kfilunm(++iPart, 14, MASS_PROTON);
		//KFit.Kinfit(Nptall, Ndecay, Idecfl, Dkind, Amsdec, Jdecay, Ldecay, Idecay, IfZfree);
		int nDecays = 1;  //only one decay, pi0 -> 2g
		Int_t Nptall = iPart + nDecays + 1, Idecfl = 0, IfZfree = 1;
		Int_t Dkind[nDecays];
		Double_t Amsdec[nDecays];
		Int_t Jdecay[nDecays];
		Int_t Ldecay[nDecays];
		Int_t* Idecay[nDecays];
		// decay pi0 -> 2g
		Dkind[0] = 7;  // Geant ID pi0
		Amsdec[0] = MASS_PIZERO;
		Jdecay[0] = iPart + 0/*i*/ + 1;  // current number of particle out of total amount of particles
		Ldecay[0] = 2;  // number of daughter particles
		for (int i = 0; i < nDecays; i++)
			// number of daughter particle
			for (int j = 0; j < Ldecay[0]; j++)
				Idecay[i][j] = j+1;
		err = KFit.Kinfit(Nptall, nDecays, Idecfl, Dkind, Amsdec, Jdecay, Ldecay, Idecay, IfZfree);
		chisq = KFit.Chisq();
		ndf = KFit.NDF();
		...
		// g + p -> eta' p -> e+ e- g p
		nDecays = 1;  // one decay involved
		int nFS = 4;
		Nptall = nFS + nDecays + 1;
		Dkind[0] = 53;
		Amsdec[0] = MASS_ETAPRIME;
		Jdecay[0] = nFS + 0 + 1;
		Ldecay[0] = 3;
		Idecay[0][0] = 1;  // id must match the particle id used in Kfilcst() for this particle
		Idecay[0][1] = 2;
		Idecay[0][2] = 3;
		KFit.Kinfit(Nptall, nDecays, Idecfl, Dkind, Amsdec, Jdecay, Ldecay, Idecay, IfZfree);
