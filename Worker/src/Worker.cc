...
Int_t Negotiate(Int_t pid, Int_t nThreads)
{
	...
	if (instances)
		runThreads = ((4*CORES-instances) / pids.size())*4 / (nThreads/2+1);  // try to compute number of used threads regarding the total amount of running threads and the users desired thread count
	if (runThreads < 1)
		runThreads = 1;
	if (runThreads > nThreads)
		runThreads = nThreads;
...
int main(int argc, char **argv)
{
	...
	if (!Offline) {
		...
		while (!feof(ServerFile)) {
			if (!fgets(Line, sizeof(Line), ServerFile))
				break;
			Buffer[0][0] = '\0';
